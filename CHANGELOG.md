...
- `backend/exchange/kucoin_ws.py`: public futures WS. **Two-step connect**: POST `https://api-futures.kucoin.com/api/v1/bullet-public` → returns `{token, instanceServers: [{endpoint, pingInterval}]}`, then connect to `<endpoint>?token=<t>&connectId=<uuid>`. Token is one-shot per session. Topic `/contract/instrument:<sym1>,<sym2>,...` multiplexes two subjects: `mark.index.price` (~1Hz per sym) and `funding.rate` (on settle / rate change). Funding-rate messages seed `_fr_cache[canonical]`; mark-price ticks attach the cached rate + interval + next-settle from discovery. **Sharding** (load-bearing): KuCoin silently caps per-connection active symbols around ~100 — with 561 perps a single socket would only push marks for the first ~100. Fix: `SLICE_SIZE=80` → 8 parallel WS sessions (well under KuCoin's 300-connection-per-user cap). Ping: `{"method":"ping"}` every `pingInterval − 2s` (server returns `pingInterval`, typically 18s).
- `backend/config.py`: `EXCHANGES["kucoin"]` with taker 0.06% / maker 0.02% (public tier), `default_funding_interval_h=4.0` (most common bucket on KuCoin), `ws_url=""` (dynamic — resolved via bullet-public at connect time).
- `backend/main.py` + `backend/exchange/pair_discovery.py` + `backend/exchange/history.py`: KuCoin wired through startup, discovery, and ccxt (`ccxt.kucoinfutures()`). Added to the `{"price":"mark"}` params tuple for mark-price klines. `pair_discovery.py` also parallelised discovery across all venues via `ThreadPoolExecutor` — cut cold-boot discovery from serial ~15s to ~3–4s.
- `backend/exchange/base.py`: new `__init_subclass__` import-time gate — every concrete connector class must resolve to an `EXCHANGES` entry carrying `maker_fee`, `taker_fee`, and `fee_source_url` (non-empty http(s) URL). Intermediate bases like `CexWSBase` are exempt (they leave `exchange_id` blank). Rationale: half the prior connectors had undocumented fee provenance — the model-chat debate (`model-chat/2026-04-21-add-exchange-skill-redesign.md`) surfaced this as the single highest-leverage hardening. Populated `fee_source_url` for all 9 existing entries.
- `backend/scripts/verify_exchange.py` + `.claude/skills/verify-exchange/SKILL.md`: new `/verify-exchange <id>` skill + helper script. Boots one connector in isolation for N seconds, asserts the four required leg fields (`mark_price`, `funding_rate`, `funding_interval_h`, `next_funding_time`) are non-null on BTCUSDT, and flags if every symbol fell back to the default interval (smells like missing cache wiring). Companion to `/add-exchange`.
- Verified: KuCoin `/verify-exchange` passes. BTCUSDT shows all four fields live (mark=75941.23, funding_rate=-3.4e-05, interval=8h, next-settle populated). Interval distribution: 391×4h / 151×8h / 19×1h — 126/561 symbols off the 4h default, confirming the discovery interval cache is wired correctly.
- **Venue-list rationale**: KuCoin picked as Phase 14 because it's the #2 most frequently cited missing venue across the competitor survey (after OKX, already Phase 13) per `research/2026-04-21-scanner-ingestion-pipelines.md`.

## 2026-04-21 — Phase 13: OKX connector (8 exchanges live)

- `backend/exchange/okx_discovery.py`: native REST `GET https://www.okx.com/api/v5/public/instruments?instType=SWAP`. Filters `ctType=linear`, `settleCcy=USDT`, `state=live`. Canonical translation `BTC-USDT-SWAP → BTCUSDT`. 292 trading perps discovered. **Quirk**: OKX returns 403 to Python's default `User-Agent`, so discovery sends `Mozilla/5.0 RoboSpread/1.0`. No interval cache seeded at discovery — OKX's instruments endpoint does NOT publish funding interval; derived at runtime instead (see below).
- `backend/exchange/okx_ws.py`: v5 public WS at `wss://ws.okx.com:8443/ws/v5/public`. Inherits `CexWSBase`. **Dual-channel subscribe** per symbol — `{"channel":"mark-price","instId":...}` + `{"channel":"funding-rate","instId":...}` — because OKX splits mark price (pushed ~5/sec per symbol) from funding rate (pushed on settlement / rate change). Every `funding-rate` message seeds three in-memory caches keyed by canonical symbol: `fundingRate`, `fundingTime` (which is the NEXT settlement, not the previous), and a derived `interval_h = (fundingTime − prevFundingTime) / 3600000`. Every `mark-price` tick calls `state.update_leg` with the cached funding triple (None until the first funding-rate message arrives per symbol). Keepalive: CexWSBase default — literal `ping`/`pong` every 20s.
- `backend/config.py`: `EXCHANGES["okx"]` with taker 0.05% / maker 0.02% (base tier).
- `backend/main.py` + `backend/exchange/pair_discovery.py` + `backend/exchange/history.py`: OKX wired through startup, discovery, and ccxt (`ccxt.okx({"options": {"defaultType": "swap"}})`). Added to the `{"price":"mark"}` params tuple for mark-price klines.
- Verified: 656 symbols listed on 2+ venues (up from 654). BTCUSDT shows **all 8 legs live** (binance/bybit/hyperliquid/bitget/gate/mexc/aster/okx) with mark, funding rate, interval, and next-funding-time populated. OKX interval distribution after 30s runtime: 178×4h / 110×8h / 1×1h — confirmed the runtime interval derivation from `fundingTime − prevFundingTime` works for non-8h symbols (no default-interval reliance).
- **Venue list rationale**: OKX was picked first from the uainvest.com.ua roster (33 venues) because it's the unanimous top next-add across every competitor surveyed — CoinGlass, Coinalyze, Coinank, Laevitas, Amberdata, Kaiko, Tardis, Hummingbot, Freqtrade. Research brief: `research/2026-04-21-scanner-ingestion-pipelines.md`.
